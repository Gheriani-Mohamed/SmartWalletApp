    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          if (!goal.isCompleted) Text('\$${goal.remainingAmount.toStringAsFixed(2)} remaining', style: TextStyle(fontSize: 14, color: Colors.white.withOpacity(0.8))),
          if (goal.isCompleted) Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20)), child: const Text('🎉 Goal Completed!', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.green))),
        ],
      ),
    );
  }
      ...
      children: [
        ...
        SizedBox(
          ...
          child: OutlinedButton.icon(
            ...
            style: OutlinedButton.styleFrom(foregroundColor: Colors.orange, padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
          ),
        ),
      ],
    );
            ...
            onPressed: () async {
              ...
              try {
                ...
                if (mounted) {
                  ...
                  // Parse error message
                  String errorMessage = 'Failed to add money';
            ...
            onPressed: () async {
              ...
              if (amount > goal.currentAmount) {
                ...
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('Insufficient funds in saving goal'),
                      backgroundColor: Colors.red,
                    ),
                  );
                }
              ...
              try {
                ...
                if (mounted) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text('Withdrew \$${amount.toStringAsFixed(2)}'),
                ...
                if (mounted) {
                  ...
                  // Parse error message
                  String errorMessage = 'Failed to withdraw';
                  if (e.toString().contains('Insufficient funds')) {
                    errorMessage = 'Insufficient funds in saving goal';
                  } else if (e.toString().contains('Exception:')) {
                    errorMessage = e.toString().replaceAll('Exception: ', '');
                  }
